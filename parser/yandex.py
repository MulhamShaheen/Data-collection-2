...
with open('parser/token.json', 'r') as fp:
...
def search_playlists(query, log=True):
    search_result = client.search(query).playlists
    results = []
    if log:
        writer = csv.writer(open(f"logging_{query}_yandex.csv", "a+", newline="", encoding="utf-8"))
...
def download_track(song_id: int, filename: Optional[str] = None):
    track = client.tracks(song_id)[0]
    if filename is None:
